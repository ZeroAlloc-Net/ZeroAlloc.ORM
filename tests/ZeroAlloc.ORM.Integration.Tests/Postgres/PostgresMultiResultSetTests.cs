...
//
// Class name prefixed with `Postgres` to keep
// `~MultiResultSetTests` filtering only the Sqlite suite.
[Trait("Provider", "Postgres")]
public sealed class PostgresMultiResultSetTests
{
    [Fact]
    ...
    // Postgres unquoted identifiers fold to lowercase; DDL written in lowercase
    // matches that fold without surprises. The MultiResultSetRepo SQL writes
    // `Orders`/`OrderLines`/`Id`/etc. — those identifiers also fold to lowercase
    // wire-side, and `DbDataReader.GetOrdinal(<name>)` is case-insensitive on
    // Npgsql so the round-trip lands on the same columns either way.

...
//   * Cancellation_via_pre_cancelled_token_stops_streaming — pre-cancelling
//     the token before iteration deterministically surfaces an
//     OperationCanceledException on Npgsql (the mid-stream cancel idiom races
//     against the Npgsql buffer; see the per-test comment for the
//     provider-difference rationale).
//
// Class name prefixed with `Postgres` so `~StreamingTests` filters only the
// Sqlite suite (avoids the v0.6 review-flagged same-name collision).
[Trait("Provider", "Postgres")]
public sealed class PostgresStreamingTests
{
    [Fact]

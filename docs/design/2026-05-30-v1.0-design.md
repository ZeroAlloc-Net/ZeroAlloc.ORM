...
| `ZeroAlloc.ORM` | Runtime helpers (parameter binding helpers, `IAsyncDbConnectionExtensions`, exception messages), depends on `AdoNet.Async` | net10.0 | ✅ |
...
3. **`CancellationToken` parameter** — method may include exactly one `CancellationToken` parameter at any position — or no `CancellationToken` (generator emits `CancellationToken.None`). A `CancellationToken` at a non-final position is supported; the generator preserves the user's declared parameter order in the emitted partial-method signature.
...
| **ZA.Telemetry** | Pure composition at consumer seam (no ZA.ORM ActivitySource) | ✅ Consumer-composed |
...
**ZA.Telemetry — consumer-composed observability.** ZA.ORM does not ship its own
ActivitySource. Observability is a cross-cutting concern at the application layer;
the canonical pattern is for adopters to declare a repository interface decorated
with ZA.Telemetry's `[Instrument]` + `[Trace]` / `[Count]` / `[Histogram]` attributes,
implement that interface as a partial class using ZA.ORM's `[Query]` annotation,
and wire DI to the ZA.Telemetry-generated proxy that wraps the ZA.ORM implementation:
...
public sealed partial class OrderRepo(IAsyncDbConnection connection) : IOrderRepo
{
    [Query("SELECT Id, CustomerId, Total FROM Orders WHERE Id = @id")]
...
services.AddSingleton<IOrderRepo>(sp => new OrderRepoInstrumented(sp.GetRequiredService<OrderRepo>()));
```

Both generators run independently: ZA.ORM fills the partial body, ZA.Telemetry
emits the instrumented proxy. No coupling between the libraries. ZA.ORM-only
adopters (who don't want instrumentation) just skip the interface declaration.

The trade-off vs a built-in ActivitySource: ZA.ORM-only adopters get no observability
out of the box. Acceptable because observability is a cross-cutting application
concern, not a data-access concern. Adopters who want unified telemetry layer it on
...
|---|---|---|---|
| **v0.1** | 4 weeks | Core attributes, single-result returns, FlatRow on positional records, primitive parameter binding, `IAsyncDbConnection` primary-ctor injection, snapshot rig, AOT gate, ZAO001-ZAO012 | `0.1.0` |
| **v0.2** | 2 weeks | Value-object discovery (ZA.ValueObjects), enum int round-trip + `[StoreAsString]`, multi-arg domain entities | `0.2.0` |
| **v0.3** | 2 weeks | Multi-result-set via `IAsyncDbBatch`, `IAsyncEnumerable<T>` streaming, tuple-of-result-sets dispatch | `0.3.0` |
| **v0.4** | 2 weeks | `[Command]` (NonQuery / Scalar / Identity), `[StoredProcedure]` with named-tuple outputs, `RETURNING` per provider | `0.4.0` |
| **v0.5** | 1 week | Multi-column composites (`Money`), `[Materialize(Factory)]` custom resolution, nullable composite handling | `0.5.0` |
| **v0.6** | 3-4 days | documentation polish + diagnostics audit: full diagnostics catalog (ZAO013-ZAO070), `docs/diagnostics/` reference pages, ZA.Telemetry composition cookbook recipe | `0.6.0` |
| **v0.7** | 1 week | Benchmark suite, ZA.Rest collision smoke test, README, API review | `0.7.0` |
| **v1.0** | API freeze | Polish, doc-link finalization | `1.0.0` |

Total: ~13 weeks single-developer pace. Each milestone ships as an independent minor NuGet release.
...
- **Per-milestone "feature complete" review gate.** Each milestone release should have a 1-day stop where we re-check the design doc against the emit and update Sections 2-3 with any divergence.
...
- **Observability:** Pure composition at the consumer seam via ZA.Telemetry's `[Instrument]`-on-interface pattern. ZA.ORM ships no `ActivitySource` of its own — both source generators compose without coupling. (Decision adjusted 2026-05-30: was originally a built-in always-emit `ActivitySource`; switched to composition to keep ZA.ORM slim and let observability live where cross-cutting concerns belong.)

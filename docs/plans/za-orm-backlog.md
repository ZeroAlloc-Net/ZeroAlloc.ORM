...
- Phase C — ZA.Telemetry observability cookbook recipe at `docs/cookbook/observability.md`; collision smoke deferred to v0.6-CLN1 due to upstream nullable-annotation issues (#70), shipped 2026-08-07
...
- ~~v0.6-T5 — ZA.Telemetry composition cookbook recipe~~ — ✅ shipped 0.6.0 (#70)
  ...
  - Collision smoke test (`ZeroAlloc.ORM.TelemetryCollision.AotSmoke`) was attempted
    but backed out — deferred as v0.6-CLN1, now shipped (ZA.Telemetry 1.4.1).
...
### ~~v0.6-CLN1 — Re-attempt ZA.Telemetry collision smoke test~~ — ✅ shipped
...
**Shipped 2026-08-07.** Fixed upstream in **ZA.Telemetry 1.4.1**
([ZeroAlloc.Telemetry#29](https://github.com/ZeroAlloc-Net/ZeroAlloc.Telemetry/issues/29)
— `SymbolDisplayFormat.FullyQualifiedFormat` omits the `?` suffix; the generator now
adds `IncludeNullableReferenceTypeModifier`). The smoke project is back at
`tests/ZeroAlloc.ORM.TelemetryCollision.AotSmoke/`, gated by the
`telemetry-collision-smoke` job in `.github/workflows/collision-smoke.yml`.

Findings worth keeping:

- **The `Task<OrderRow?>` method is routed through the interface deliberately.**
  The original failure was a compile error on the generated proxy implementing
  the interface, so an interface-typed call is the assertion; calling the
  concrete proxy directly would not have caught it.
- **`EPC12` fires on ZA.Telemetry's generated proxy** and is suppressed via
  `NoWarn` in the smoke csproj. The generated catch reads `_ex.Message` for the
  span status then rethrows with a bare `throw;`, so nothing is swallowed — but
  ErrorProne.NET analyses generated files despite their `// <auto-generated />`
  header, which normally exempts them. Worth raising upstream if other adopters
  using ErrorProne.NET hit it.
- Cookbook recipe at `docs/cookbook/observability.md` now documents the smoke as
  an active gate rather than a pending one.
...
- ~~**v0.6-CLN1** — Re-attempt ZA.Telemetry collision smoke once upstream fixes nullable annotations.~~ — ✅ shipped 2026-08-07 (ZA.Telemetry 1.4.1)
